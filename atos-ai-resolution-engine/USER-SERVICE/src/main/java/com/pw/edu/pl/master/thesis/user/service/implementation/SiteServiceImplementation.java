...
import com.pw.edu.pl.master.thesis.user.client.ProjectClient;
...
public class SiteServiceImplementation implements SiteService {
    ...
    public SiteResponse addNewSite(AddSiteRequest request) {
        ...
        // Mirror Jira projects into PROJECT-SERVICE local DB for the current authenticated user.
        projectClient.syncAllFromJira();
    ...
    public SiteResponse updateSiteName(Long siteId, String newSiteName) {
        ...
        refreshSiteProjectsFromJira(site, site.getBaseUrl(), jiraEmail, tokenPlain);
        projectClient.syncAllFromJira();
    ...
    public SiteResponse updateSiteURL(Long siteId, String newBaseURL) {
        ...
        refreshSiteProjectsFromJira(site, normalized, jiraEmail, tokenPlain);
    ...
    private void refreshSiteProjectsFromJira(Site site, String baseUrl, String jiraEmail, String tokenPlain) {
        List<ProjectSummary> remote = listAllProjects(baseUrl, jiraEmail, tokenPlain);
        ...
        for (ProjectSummary project : remote) {
            String key = trimToNull(project.getKey());
            String name = trimToNull(project.getName());
            String jiraId = trimToNull(project.getId());
            if (StringUtils.isBlank(key) || StringUtils.isBlank(name)) continue;

            seenKeys.add(key);

            SiteProject existingProject = byKey.get(key);
            if (existingProject == null) {
                siteProjectRepo.save(SiteProject.builder()
                        .site(site)
                        .jiraId(jiraId)
                        .key(key)
                        .name(name)
            ...
            boolean dirty = false;
            if (!StringUtils.equals(existingProject.getName(), name)) {
                existingProject.setName(name);
                dirty = true;
            }
            if (!StringUtils.equals(existingProject.getJiraId(), jiraId)) {
                existingProject.setJiraId(jiraId);
                dirty = true;
            }
            if (dirty) {
                siteProjectRepo.save(existingProject);
            }
        }

        for (SiteProject project : existing) {
            if (!seenKeys.contains(project.getKey())) {
                siteProjectRepo.delete(project);
            }
        }
    }
