...
public class GitHubServiceImplementation implements GitHubService {
    ...
    @Override
    public String createBranch(String token, String repoName, String baseBranch, String newBranch) throws IOException {
        GHRepository repo = getRepo(token, repoName);
    ...
    @Override
    public GitHubFileData getFile(String token, String repoName, String branch, String filePath) throws IOException {
        GHRepository repo = getRepo(token, repoName);
    ...
    @Override
    public void updateFile(String token, String repoName, String branch, String filePath, String newContent, String commitMessage) throws IOException {
        GHRepository repo = getRepo(token, repoName);
        GHContent file = repo.getFileContent(filePath, branch);
        repo.createContent()
                .content(newContent)
    ...
    @Override
    public String createPullRequest(String token, String repoName, String branch, String baseBranch, String title, String description) throws IOException {
        GHRepository repo = getRepo(token, repoName);
    ...
    @Override
    public List<String> listRepositoryFiles(String token, String repoName, String branch) throws IOException {
        GHRepository repo = getRepo(token, repoName);
    ...
    private GHRepository getRepo(String token, String repoName) throws IOException {
        ...
        return buildClient(token).getRepository(effectiveRepo);
    }

    private GitHub buildClient(String token) throws IOException {
        String effectiveToken = token == null || token.isBlank() ? properties.getToken() : token.trim();
        if (effectiveToken == null || effectiveToken.isBlank()) {
            throw new ValidationException("GitHub token is not configured");
        }
        return new GitHubBuilder()
                .withOAuthToken(effectiveToken)
                .build();
    }
}
