...
export class AdminJiraConfigComponent implements OnInit {
  ...
  saveConfig(): void {
    ...
    const payload: SiteCreatePayload = {
      ...
      hostPart: this.configForm.controls.siteName.value.trim(),
      baseUrl: this.configForm.controls.baseUrl.value.trim() || undefined,
      username: this.configForm.controls.username.value.trim(),
      jiraToken: this.configForm.controls.jiraToken.value.trim()
    };
    this.isSaving = true;
